...
function calcPrice(
  ...
  if (!priceA || !priceB) {
    throw new Error(
      'calcPrice 함수의 첫 번째와 두 번째 인수는 필수 입력값입니다.'
    );
  }

  return `총 합 = ${priceA + priceB + priceC + priceD}`;
}
...
/* function rem(pxValue, base = 16) {
  ...
  if (typeof pxValue === 'string') {
    if (pxValue.includes('px')) {
      result = `${parseInt(pxValue,10) / base}`;
    } else {
      throw new Error('px단위로 입력해주세요');
    }
  }
  return `${result}rem`;
}
...
function rem(pxValue, base = 16) {
  ...
  typeof value === 'string' && (base = parseInt(base, 10));
