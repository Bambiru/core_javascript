/* ------------------------------ */
/* Array's Methods                */
/* ------------------------------ */

// Array.isArray

// console.log(Array.isArray([])); // true

/* 일반함수 */
const isArray = (data) =>
  // Array.isArray(data)
  Object.prototype.toString.call(data).slice(8, -1).toLowerCase() === 'array';

const isNull = (data) =>
  Object.prototype.toString.call(data).slice(8, -1).toLowerCase() === 'null';

function normalIsArray(data) {
  // return Array.isArray(data);
  return (
    Object.prototype.toString.call(data).slice(8, -1).toLowerCase() === 'array'
  );
}

// const arr = [10, 100, 1000, 10_000];
const people = [
  {
    id: 0,
    name: '김다영',
    job: '프론트엔드 개발자',
    age: 25,
    imageSrc: 'ASdk131',
  },
  {
    id: 1,
    name: '김충만',
    job: '치킨집사장',
    age: 51,
    imageSrc: 'asFkzo23',
  },
  {
    id: 2,
    name: '조윤주',
    job: '화가',
    age: 12,
    imageSrc: 'Gzoskq13',
  },
  {
    id: 3,
    name: '송현규',
    job: '모델',
    age: 25,
    imageSrc: 'aabaq23',
  },
];

/* 대상이 배열이어야만 true를 반환한다. */

/* 요소 순환 ---------------------------- */

// forEach => 반환값이 없다.
// [10,100,1000,10_000]
/* arr.forEach((item, index) => {
  // console.log(item, index);
});
for (let i = 0; i < arr.length; i = i + 1) {
  console.log(arr[i], i);
} */

// people.forEach((item) => console.log(item.job));
people.forEach((item) => {
  // console.log(item.job);
});

const span = document.querySelectorAll('span');
span.forEach((item) => {
  item.addEventListener('click', function () {
    this.style.color = 'red';
  });
});

// 여러 항목들을 이벤트 바인딩 제일 효과적인가 ? console.error('아니다',아니다)
// const first = document.querySelector('.first');
// const second = document.querySelector('.second');
// const third = document.querySelector('.third');
/* 원형 파괴 ----------------------------- */

// push
// pop
// unshift
// shift
// reverse
// splice
// sort
const arr = [10, 100, 1000, 10_000];
// const reverseArray = arr.reverse();

const reverseArray = arr.toReversed(); // 원형 파괴 x 2023

// console.log( reverseArray );

// console.log(arr.splice(1, 2, 'javascript', 'css', 'react'));
// console.log(arr);
// console.log(arr.toSpliced(1, 2, 'javascript', 'css', 'react')); // 원형 파괴 x 2023

/* const a = arr.sort((a, b) => {
  return b - a;
}); */
const a = arr.toSorted((a, b) => {
  // 원형 파괴 x 2023
  return b - a;
});
// console.log(a);
/* 새로운 배열 반환 ------------------------ */

// concat
// slice
// toSorted
// toReversed
// toSpliced
// map

const card = people.map((item, index) => {
  const { job, imageSrc, alt, name, age } = item;
  return /* html */ `
  <div class='userCard card${index}'>
    <div class='imageField'>
      <img src="${imageSrc}" alt="${alt}" />
    </div>
    <span>이름 : ${name}</span>
    <span>나이 : ${age}</span>
    <span>직업 : ${job}</span>
  </div>`;
});

card.forEach((item) => {
  // document.body.insertAdjacentHTML('beforeend', item);
});

const newAge = people.map((item) => item.age - 2);
// console.log(newAge);

/* 요소 포함 여부 확인 ---------------------- */

// indexOf
// lastIndexOf
// includes

/* 요소 찾기 ------------------------------ */

// find
// findIndex
const findUser = people.find((item) => {
  return item.age < 30;
});
// console.log(findUser);

const findUserIndex = people.findIndex((item) => {
  return item.age < 30;
});
// console.log(findUserIndex);

/* 요소 걸러내기 --------------------------- */

// filter
const 사장님빼고모여 = people.filter((item) => {
  return item.age < 30;
});
// console.log(사장님빼고모여);
/* 요소별 리듀서(reducer) 실행 -------------- */

// reduce
const totalAge = people.reduce((acc, cur) => acc + cur.name, 0);

const template = people.reduce((htmlCode, cur) => {
  return htmlCode + `<div>${cur.name} : ${cur.age}살</div>`, '';
});

document.body.insertAdjacentHTML('beforeend', template);
// console.log(totalAge);
// reduceRight

/* string ←→ array 변환 ------------------ */

// split
// join

const str = '종명 예진 현주 지인 훈 은원';

// split : 문자를 배열로 만들어준다.
const stringToArray = str.split(' ');
console.log(stringToArray);

// join : 배열을 문자로 만들어준다.
const arrayToString = stringToArray.join('-');
console.log(arrayToString);
